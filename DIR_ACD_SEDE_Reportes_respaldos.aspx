<%@ Page Language="VB" AutoEventWireup="false" CodeFile="DIR_ACD_SEDE_Reportes_respaldos.aspx.vb" Inherits="DIR_ACD_SEDE_Reportes_respaldos" MasterPageFile="mp_dir_academico_sedes.master" %>
...
<%--                                    <asp:DropDownList ID="ddl_respaldo_de" CssClass="form-control btn btn-default dropdown-toggle mt-1"  runat="server">
                                ...
                                <asp:DropDownList ID="ddl_sede" CssClass="form-control btn btn-default dropdown-toggle mt-1"  runat="server" AutoPostBack="True" DataSourceID="sql_ds_sede" DataTextField="NombreSede" DataValueField="idSede"></asp:DropDownList>
                                <asp:SqlDataSource ID="sql_ds_sede" runat="server" ConnectionString="<%$ ConnectionStrings:unitepc_boliviaConnectionString %>" SelectCommand="SELECT * FROM [tb_sedes] where idSede=@sede">
                                    <SelectParameters>
                                        <asp:ControlParameter ControlID="txt_sede" Name="sede" PropertyName="Text" />
                                    </SelectParameters>
                                </asp:SqlDataSource>
                                <asp:DropDownList ID="ddl_carrera" runat="server" CssClass="form-control btn btn-default dropdown-toggle mt-1"  AutoPostBack="True" DataSourceID="sql_ds_carrera" DataTextField="CARDES" DataValueField="CARSIGLA" AppendDataBoundItems="True">
             ...
             <div class="col-md-8 p-3 mb-2 bg-white text-black border border-dark rounded ">
                 ...
                 <asp:GridView ID="GridView1" runat="server" AutoGenerateColumns="False" DataSourceID="sql_ds_horas" Font-Size="8pt" ShowFooter="True">
                     <Columns>
                         ...
                         <asp:BoundField DataField="hora_inicio" HeaderText="hora_inicio" SortExpression="hora_inicio"></asp:BoundField>
                         <asp:BoundField DataField="hora_fin" HeaderText="hora_fin" SortExpression="hora_fin" />
                         <asp:BoundField DataField="hra_ideal" HeaderText="hra_ideal" ReadOnly="True" SortExpression="hra_ideal" />
                         <asp:BoundField DataField="diferencia" HeaderText="diferencia" ReadOnly="True" SortExpression="diferencia" />
                         <asp:BoundField DataField="observaciones" HeaderText="observaciones" SortExpression="observaciones" />
                     </Columns>
                              </asp:GridView>
                              <asp:SqlDataSource ID="sql_ds_horas" runat="server" ConnectionString="<%$ ConnectionStrings:unitepc_boliviaConnectionString %>" SelectCommand="SELECT ti.ci_doc, Materia, tema_avance, carrera, cant_est, hr_ingreso, hr_salida, DATEDIFF(SS, hr_ingreso, hr_salida) / 60 AS MINUTOS,th.hora_inicio ,th.hora_fin,DATEDIFF(ss,th.hora_inicio ,th.hora_fin)/60 as hra_ideal ,((DATEDIFF(ss,th.hora_inicio ,th.hora_fin)/60 )-(DATEDIFF(SS, hr_ingreso, hr_salida) / 60))as diferencia, observaciones 
FROM tb_ing_sal AS ti 
join tb_horario th on ti.id_horario = th.id_horario 
WHERE (ti.ci_doc = @cidoc) AND (carrera = @carrera) AND (hr_ingreso BETWEEN @fini AND @fin+1)  
ORDER BY TI.id_ing_sal" ProviderName="<%$ ConnectionStrings:unitepc_boliviaConnectionString.ProviderName %>">
                                  <SelectParameters>
