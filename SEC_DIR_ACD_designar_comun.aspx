<%@ Page Language="VB" AutoEventWireup="false" CodeFile="SEC_DIR_ACD_designar_comun.aspx.vb" Inherits="SEC_DIR_ACD_designar_comun" MasterPageFile="~/mp_sec_dir_academico.master" %>
                            ...
                            <div class="col-md-6 p-3 mb-2 bg-white text-white border border-dark rounded ">
                              <div style="background-color:#FFCC00" class="text-md-center  rounded text-bold " role="alert">
                               Seleccione la clase y al Docente en la Sgte. Lista para Asignarle una materia Comun
                                ...
                                <asp:DropDownList ID="ddl_clases_prog" runat="server" AppendDataBoundItems="True" AutoPostBack="True" CssClass="form-control btn btn-default dropdown-toggle mt-1" DataSourceID="sql_ds_clases_prog" DataTextField="Column1" DataValueField="id_horario">
                                                       <asp:ListItem Value="0">Seleccione una Clase Programada</asp:ListItem>
                                                   </asp:DropDownList>
                                               </ContentTemplate>
                                           </asp:UpdatePanel>

                                           <asp:SqlDataSource ID="sql_ds_clases_prog" runat="server" ConnectionString="<%$ ConnectionStrings:unitepc_boliviaConnectionString %>" SelectCommand="select th.id_horario ,(replace((tp.nombres+' '+tp.primerApellido +' '+tp.segundoApellido ),'a a a','--* Sin Asignar Docente *--')+'--&gt; MATERIA: '+p.Materia+'---&gt; Dia:'+th.dia +'---&gt;Tipo de Clase: '+ th.tipo_clase+'---&gt;Horas:'+th.hora_inicio +' a '+th.hora_fin  ) 
from tb_horario th
join tb_designacion d on d.id_horario=th.id_horario
join  tb_personal tp on th.ci_doc = tp.ci 
join planes_estudios p on d.id_materia=p.id_plan
where th.gestion='2-2022' and tp.gestion='2-2022' and d.carrera=@carrera and th.sede=@sede" UpdateCommand="UPDATE tb_horario SET designado = 'Si' WHERE (id_horario = @id_horario)">
                                               <SelectParameters>
                                ...
                                <asp:SqlDataSource ID="sql_ds_carreras_origen" runat="server" ConnectionString="<%$ ConnectionStrings:unitepc_boliviaConnectionString %>" SelectCommand="SELECT * FROM [tb_carreras]">
                                </asp:SqlDataSource>
