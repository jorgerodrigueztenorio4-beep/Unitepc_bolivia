<%@ Page Language="VB" AutoEventWireup="false" CodeFile="seguimiento_el_alto1.aspx.vb" Inherits="seguimiento_el_alto" MasterPageFile="mp_el_alto.master" %>

<asp:Content ID="Content1" runat="server" contentplaceholderid="ContentPlaceHolder1">
   

     <div class="alert alert-success mt-1 text-md-center text-bold  " role="alert">
    <h3>Seguimiento Docente El Alto</h3>
        </div> 
...
<asp:ListItem>70</asp:ListItem>
 ...
 WHERE 
(th.ci_doc =@ci) AND (th.gestion = '2-2023') AND (th.sede = '4') and tp.gestion='2-2023' and ta.gestion='2-2023'
AND (CONVERT (time, SYSDATETIME(), 114) &gt;= DATEADD(MINUTE, - 5, CONVERT (time, CONVERT (time, th.hora_inicio, 114)))) 
 AND (CONVERT (time, SYSDATETIME(), 114) &lt;= DATEADD(MINUTE, 45, CONVERT (time, CONVERT (time, th.hora_inicio, 114))))
   AND th.dia =(SELECT (CASE DATENAME(dw,getdate())
     when 'Lunes' then 'Lunes'
...
WHERE tb_horario .id_horario = @id_hor and tb_ing_sal.id_ing_sal =@id_ingsal  and convert(time,getdate() )&gt;tb_horario.hora_fin" ProviderName="<%$ ConnectionStrings:unitepc_boliviaConnectionString.ProviderName %>">
                            ...
                            <asp:SqlDataSource ID="sql_ing_sal" runat="server" ConnectionString="<%$ ConnectionStrings:unitepc_boliviaConnectionString %>" InsertCommand="INSERT INTO tb_ing_sal(ci_doc, hr_ingreso, Materia, carrera, sede, id_horario, aula_o_lab, bloque, sigla_materia, ip_marcado) VALUES (@ci, GETDATE(), @materia, @carrera, '4', @idhorario, @aula, @bloque, @siglamat, @ip)" ProviderName="<%$ ConnectionStrings:unitepc_boliviaConnectionString.ProviderName %>" SelectCommand="select * from [tb_ing_sal]
...
">
                            ...
                            <asp:SqlDataSource ID="sql_ing_sal0" runat="server" ConnectionString="<%$ ConnectionStrings:unitepc_boliviaConnectionString %>" InsertCommand="INSERT INTO tb_ing_sal(ci_doc, hr_ingreso, Materia, carrera, sede, id_horario, aula_o_lab, bloque, sigla_materia, ip_marcado) VALUES (@ci, @horaingreso, @materia, @carrera, '4', @idhorario, @aula, @bloque, @siglamat, @ip)" ProviderName="<%$ ConnectionStrings:unitepc_boliviaConnectionString.ProviderName %>" SelectCommand="select * from [tb_ing_sal]
where ci_doc =@ci and hr_salida is null
">
                                <InsertParameters>
                                    <asp:ControlParameter ControlID="txt_ci" Name="ci" PropertyName="Text" />
