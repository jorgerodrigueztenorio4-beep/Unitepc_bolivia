<%@ Page Language="VB" AutoEventWireup="false" CodeFile="DIR_CAR_programar_horario2Copy.aspx.vb" Inherits="DIR_CAR_programar_horario2Copy" MasterPageFile="mp_dir_car.master" %>
    	...
    	<div class="row">
                            ...
                            <div class="col-md-4 p-1 mb-2 bg-white text-white border border-dark rounded ">
                              <div style="background-color:#ffc107" class="text-md-center  rounded text-bold " role="alert">
                                ...
                                <asp:DropDownList ID="ddl_sede" CssClass="form-control btn btn-default border-dark  dropdown-toggle mt-1" runat="server" DataSourceID="sql_ds_sede" DataTextField="NombreSede" DataValueField="idSede" AutoPostBack="True">
                                ...
                                <asp:DropDownList ID="ddl_carrera" CssClass="form-control btn btn-default border-dark  dropdown-toggle mt-1" runat="server" DataSourceID="sql_ds_carrera" DataTextField="CARDES" DataValueField="CARSIGLA">
                                </asp:DropDownList>
...
where CARSIGLA =@siglacar">
    ...
    <%--Tipo de Clase--%>
              ...
              <div class="col-md-4 p-3 mb-2 bg-white text-white border border-dark rounded ">
                         ...
                         <asp:GridView ID="GridView2" runat="server" AutoGenerateColumns="False" DataSourceID="sql_ds_libre">
                                                   <Columns>
                                                       <asp:BoundField DataField="id_nomal" HeaderText="id_nomal" ReadOnly="True" SortExpression="id_nomal" InsertVisible="False" />
                                               ...
                                               <asp:SqlDataSource ID="sql_ds_libre" runat="server" ConnectionString="<%$ ConnectionStrings:unitepc_boliviaConnectionString %>" SelectCommand="WITH horarios_ocupados AS (SELECT th.hora_inicio, th.hora_fin, a.nom_aula_lab, a.id_nomal, a.capacidad FROM tb_horario AS th INNER JOIN tb_aula AS a ON th.aula = a.id_nomal WHERE (th.gestion = '1-2025') AND (th.sede = @sede) AND (th.bloque = @Bloque) AND (th.dia = @dia) AND (a.gestion = '1-2024')), rango_horas AS (SELECT nom_aula_lab, id_nomal, capacidad, hora AS hora_inicio, (SELECT MIN(hora) AS Expr1 FROM (SELECT hora_inicio AS hora FROM horarios_ocupados AS horarios_ocupados_6 UNION SELECT hora_fin AS hora FROM horarios_ocupados AS horarios_ocupados_5 UNION SELECT '06:45' AS hora UNION SELECT '22:15' AS hora) AS posibles_horas WHERE (hora &gt; horas.hora)) AS hora_fin FROM (SELECT hora_inicio AS hora, nom_aula_lab, id_nomal, capacidad FROM horarios_ocupados AS horarios_ocupados_4 UNION SELECT hora_fin AS hora, nom_aula_lab, id_nomal, capacidad FROM horarios_ocupados AS horarios_ocupados_3 UNION SELECT '06:45' AS hora, nom_aula_lab, id_nomal, capacidad FROM horarios_ocupados AS horarios_ocupados_2 UNION SELECT '22:15' AS hora, nom_aula_lab, id_nomal, capacidad FROM horarios_ocupados AS horarios_ocupados_1) AS horas) SELECT DISTINCT rh.id_nomal, rh.nom_aula_lab, rh.hora_inicio, rh.hora_fin, rh.capacidad FROM rango_horas AS rh LEFT OUTER JOIN horarios_ocupados AS ho ON rh.nom_aula_lab = ho.nom_aula_lab AND rh.hora_inicio = ho.hora_inicio AND rh.hora_fin = ho.hora_fin WHERE (rh.hora_inicio &gt;= '06:45') AND (rh.hora_fin &lt;= '22:15') AND (rh.hora_fin IS NOT NULL) AND (NOT EXISTS (SELECT 1 AS Expr1 FROM horarios_ocupados AS ho WHERE (nom_aula_lab = rh.nom_aula_lab) AND (hora_inicio &lt;= rh.hora_inicio) AND (hora_fin &gt; rh.hora_inicio))) ORDER BY rh.nom_aula_lab, rh.hora_inicio">
                                                   <SelectParameters>
...
">
           ...
           <asp:TextBox ID="txt_horas" runat="server" Visible="False"></asp:TextBox>
                 ...
                 <asp:SqlDataSource ID="SQL_DS_HABILITAR_HORARIO" runat="server" ConnectionString="<%$ ConnectionStrings:unitepc_boliviaConnectionString %>" SelectCommand="SELECT * FROM [tb_habilitaciones_horarios] where SEDE =@sede and carrera =@carrera">
               <SelectParameters>
                   <asp:ControlParameter ControlID="txt_sede" Name="sede" PropertyName="Text" />
