<%@ Page Language="VB" AutoEventWireup="false" CodeFile="DIR_ACD_SEDE_eliminar_horario.aspx.vb" Inherits="DIR_ACD_SEDE_eliminar_horario" MasterPageFile="~/mp_dir_academico_sedes.master" %>
                              ...
                              <div style="background-color:#FFCC00" class="text-md-center  rounded text-bold " role="alert">
                               Lista de Horarios para Eliminar
                                 ...
                                 <div class="input-group mb-1">
                                     ...
                                     <asp:SqlDataSource ID="sql_sede" runat="server" ConnectionString="<%$ ConnectionStrings:unitepc_boliviaConnectionString %>" SelectCommand="SELECT * FROM [tb_sedes] where idSede=@id_sede">
                                         <SelectParameters>
                                     ...
                                     <asp:SqlDataSource ID="sql_horarios" runat="server" ConnectionString="<%$ ConnectionStrings:unitepc_boliviaConnectionString %>" SelectCommand="select d.id_designacion ,h.id_horario ,  replace((p.nombres+' '+p.primerApellido +' '+p.segundoApellido ),'a a a','SIN DOCENTE')as docente ,d.horas ,h.tipo_clase,m.Materia ,h.dia ,h.hora_inicio ,h.hora_fin,h.grupo,a.nom_aula_lab   ,d.carrera,m.Semestre  ,h.gestion ,h.fecha_registro ,ci_usuario ,s.NombreSede 
  from tb_horario h
...
join tb_designacion d on d.id_horario=h.id_horario 
join tb_personal p on h.ci_doc = p.ci 
where h.gestion  ='2-2022' and d.id_designacion =@id_designacion" DeleteCommand="DELETE FROM tb_horario WHERE (id_horario = @id_horario)">
                                         <DeleteParameters>
