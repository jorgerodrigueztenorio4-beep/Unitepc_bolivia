<%@ Page Language="VB" AutoEventWireup="false" CodeFile="RR_HH_Reportes_respaldos2.aspx.vb" Inherits="RR_HH_Reportes_respaldos2" MasterPageFile="mp_rr_hh.master" %>
...
where th.sede =@sede and p.codcar =@sigcar   and th.gestion ='2-2023' and tp.gestion ='2-2023'    group by nombres ,primerApellido,segundoApellido,ci_doc order by docente">
                                    <SelectParameters>
                                ...
                                <asp:DropDownList ID="ddl_tipo_clase"  CssClass="form-control btn btn-default dropdown-toggle mt-1"  runat="server">
             ...
             <div class="col-md-8 p-3 mb-2 bg-white text-black border border-dark rounded ">
                 ...
                 <asp:GridView ID="GridView1" runat="server" AutoGenerateColumns="False" DataSourceID="sql_ds_horas" Font-Size="8pt" ShowFooter="True">
                     <Columns>
                         ...
                         <asp:BoundField DataField="MINUTOS" HeaderText="horas" SortExpression="MINUTOS" ReadOnly="True" />
                         <asp:BoundField DataField="observaciones" HeaderText="observaciones" SortExpression="observaciones"></asp:BoundField>
                     </Columns>
                              </asp:GridView>
                              <asp:SqlDataSource ID="sql_ds_horas" runat="server" ConnectionString="<%$ ConnectionStrings:unitepc_boliviaConnectionString %>" SelectCommand="Select ti.ci_doc ,ti.Materia ,ti.tema_avance ,ti.carrera ,cant_est ,hr_ingreso ,hr_salida ,convert(integer,round((((convert(decimal,datediff (SS,TI.hr_ingreso   ,TI.hr_salida ))/60)/45)),0)) AS MINUTOS,ti.observaciones 
FROM tb_ing_sal ti
where TI.ci_doc =@ci and TI.carrera =@carrera and  TI.hr_ingreso BETWEEN @fini and (@fin +1)
order by TI.id_ing_sal" InsertCommand="INSERT INTO tb_ajustes(ci_doc, fecha_ini, fecha_fin, carrera, ajuste, modificado_por, fecha_ajuste, sede, tipo_clase) VALUES (@cidoc, @feha_ini, @fecha_fin, @carrera, @ajuste, @modpor, GETDATE(), @sede, @tipoclase)" ProviderName="<%$ ConnectionStrings:unitepc_boliviaConnectionString.ProviderName %>">
                                  <InsertParameters>
